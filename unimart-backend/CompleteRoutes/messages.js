  ...
  async (req, res) => {
    try {
      ...
      if (listing.seller._id.toString() === req.user._id.toString()) {
        return res
          .status(400)
          .json({ message: "Cannot message yourself about your own listing" });
      }
      ...
      res
        .status(500)
        .json({ message: "Server error while starting conversation" });
    }
  }
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Server error while marking conversation as read" });
    }
  }
);
