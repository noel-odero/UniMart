...
const Browse = () => {
    ...
    return (
        ...
        {/* Header */}
        <div className="mb-8">
            <h1 className="text-3xl font-bold text-brown-800 mb-2 text-shadow">Browse Marketplace</h1>
            <p className="text-brown-600">Discover amazing items from students across African universities.</p>
        </div>
        ...
        <div className={`grid gap-6 ${viewMode === "grid" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-1"}`}>
            ...
            <Card key={listing.id} className="card-3d bg-gradient-to-br from-tan-50 to-brown-50 border-brown-200/50 hover:shadow-lg transition-all duration-300 transform hover:scale-105">
                ...
                <div className="flex justify-between items-start mb-2">
                    ...
                    variant="ghost"
                    size="sm"
                    className={`p-1 ${listing.isFavorited ? "text-red-500" : "text-brown-400"} hover:text-red-500`}
                    >
                    <Heart className={`w-4 h-4 ${listing.isFavorited ? "fill-current" : ""}`} />
                    </Button>
                </div>
                <CardTitle className="text-lg text-brown-800 line-clamp-1">{listing.title}</CardTitle>
                <CardDescription className="text-brown-600 line-clamp-2">{listing.description}</CardDescription>
                </CardHeader>
                <CardContent>
                <div className="space-y-3">
                    ...
                    <Button size="sm" className="flex-1 bg-brown-600 hover:bg-brown-700">
                        View Details
                    </Button>
        ...
        {sortedListings.length === 0 && (
            ...
            <h3 className="text-xl font-semibold text-brown-700 mb-2">No items found</h3>
